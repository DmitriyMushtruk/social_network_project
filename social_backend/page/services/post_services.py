...
def get_feeds(user: User, current_page: Page) -> Response:
    condition = (
        Q(page__owner=user, reply_to__isnull=True, page__unblock_date__isnull=True) | # Обычные посты собственных страниц
        Q(page__followers=current_page, reply_to__isnull=True, page__unblock_date__isnull=True) | # Обычные посты страниц, на которые подписана текущая страница
        Q(page__owner=user, reply_to__isnull=False, reply_to__page__is_private=False,  page__unblock_date__isnull=True) # Посты-репосты собственных страниц
    )
